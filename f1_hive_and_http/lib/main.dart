...
* 주로 로컬에 데이터를 저장하는 데 사용되며, 모바일 및 애플리케이션에서 빠르고 효율적인 데이터 저장 및 검색 기능을 제공한다.
* hive의 구조는 간단하고, 사용이 용이하며, 성능이 뛰어나기 때문에 많은 flutter 개발자들 사이에서 인기가 높다.
*
* 주요 특징
* 1. 경량 및 빠른 성능
...
*
* 4. 비동기 지원
*   hive는 비동기 작업을 지원하며 ui 스레드와 충돌 없이 데이터를 읽고 쓸 수 있따.
*   이는 사용자의 경험을 향상시키는데 도움이 된다.
*
* 5. 데이터 암호화
*   hive는 데이터를 안전하게 보호하기 위한 암호화 기능을 제공한다.
*   개인 정보나 민감한 데이터를 저장할 때 유용하다. (그러나 저장하지 않아야 함)
* */
...
void main() async{
  ...
  * 다양한 flutter 기능과 플러그인을 정상적으로 사용할 수 있또록 환경을 설정하기 위함.
  *
  * 주요 기능
  * flutter 프레임 워크와 퉁신준비 : flutter ui와 애플리케이션의 다른 부분간의 연결을 설정한다.
  * 비동기 작업 수행 : 애플리케이션의 초기화 과정에서 비동기 작업을 수행할 때 , flutter의 위젯 트리와 이벤트 루프가 준비되어 있어야 한다.
  * 플러그인 사용 : flutter에서 제공하는 많은 플러그인은 flutter의 바인딩이 초기화된 후에만 제대로 동작하게 된다.
  * 위젯 트리의 상태관리 : flutter 애플리케이션 구조에서 위젯 트리와 상태 관리가 잘 작동하기 위해서는 초기화가 필수적이다.
  * */
