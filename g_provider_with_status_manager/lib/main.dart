...
*
* 주요 기능
* 상태관리 : provider는 애플리케이션의 상태를 관리하고, 상태가 변경될 때 ui를 자동으로 업데이트한다.
...
void main() {
  ...
  runApp(
    ...
    *   이 클래스는 상태가 변경될때 notifyListeners() 메서드를 호출하여 구독중인 위젯에 알림을 보낸다.
    *
    * 2. 의존성 주입
    *   ChangeNotifierProvider를 사용하여 애플리케이션의 위젯 트리에 상태를 주입할 수 있따.
    *   이를 통행 하위 위젯들이 해당 상태에 접근할 수 있게 된다.
    *
    * 3. 자동 ui 업데이트 :
    *   ChangeNotifierProvider를 통해 상태가 변경되면
    *   이를 구독하고 있는 consumer 또는 seletor와 같은 위젯이 자동으로 리렌더링 된다.
    *
    * */
      ChangeNotifierProvider(
