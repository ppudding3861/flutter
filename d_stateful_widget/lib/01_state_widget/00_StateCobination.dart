...
/*
* StatefulWidget
* flutter에서 상태를 가지는 위젯을 저의하는 데 사용되는 클래스이다.
* 이 위젯은 내부 상태가 변경될 수 있으며, 상태의 따라 ui가 동적으로 업데이트 된다.
*
* 특징
* 1. 상태 변화 가능 : StatefulWidget은 사용자 입력, 애니메이션, 네트워크 요청 등으로 인해
*                   상태가 변경될 수 있으며 상태가 변경될 때마다, build 메서드가 호출되어  ui를 업데이트 한다.
* 2. 상태관리 : StatefulWidget은 두 부분으로 나뉘게 되는데 하나는 위젯 자체를 정의하는 StatefulWidget 클래스이고
*             다른 하나는 ui의 사태를 관리하는 State 클래스이다. 두 클래스는 연결되 어 있으며 State 클래스에서 상태를 관리한다.
...
* 3. didChangeDependencies : initState 다음에 호출되며, 위젯의 의존성이 변경되 때마다 호출된다.
*                             주로 inheritedWidget에 의존하는 경우 해당 위젯이 변경될 때 필요한 작업을 할 수 있다.
* 4. buile : 위젯이 화면에 그려질 때 호출된다. ui를 구성하는 코드가 이곳에 위치하며 상태가 변경되면 이 메소드가 다시 호출되어 ui를 갱신한다.
* 5. didUpdateWidget : 부모 위젯이 재구성될 때 호출되며, 이전 위젯과 새로운 위젯을 비교하여 필요한 작업을 수행 할 수 있다.
...
*
* 생명주기 요약
* StatefulWidget의 생명주기는 위젯이 생성되고, 변경되고, 제거되는 과정을 관리한다. 이를 통행 개발자는 상태 변화를 효율적으로 처리하고,
* 사용자에게 일관된 경험을 제공할 수 있다. 생명주기 메소드를 적절히 활영하여 성능과 사용자 경험을 제공할 수 있따.
*
* 0. setState // 기본
* 1. provider
* 2. Bloc     // 비즈니스로직을 컴포넌트로 분리하는거
* 3. Riverpod
* */
